...
/**
 * Host for the Pen-related blocks in Boundlo! 1
 * @param {Runtime} runtime - the runtime instantiating this block package.
 * @constructor
 */
class Boundlo1PenBlocks {
    constructor (runtime) {
        /**
    ...
    _clampPenSize (requestedSize) {
        return MathUtil.clamp(
            requestedSize,
            Boundlo1PenBlocks.PEN_SIZE_RANGE.min,
            Boundlo1PenBlocks.PEN_SIZE_RANGE.max
        );
    }
     ...
     */
    _getPenState (target) {
        let penState = target.getCustomState(Boundlo1PenBlocks.STATE_KEY);
        if (!penState) {
            penState = Clone.simple(Boundlo1PenBlocks.DEFAULT_PEN_STATE);
            target.setCustomState(Boundlo1PenBlocks.STATE_KEY, penState);
        }
        return penState;
    }
    ...
    _onTargetCreated (newTarget, sourceTarget) {
        if (sourceTarget) {
            const penState = sourceTarget.getCustomState(Boundlo1PenBlocks.STATE_KEY);
            if (penState) {
                newTarget.setCustomState(Boundlo1PenBlocks.STATE_KEY, Clone.simple(penState));
                if (penState.penDown) {
    ...
    setPenColorToColor (args, util) {
        ...
        // Set the legacy "shade" value the same way boundlo 2 did.
    ...
    /* LEGACY OPCODES */
    /**
     * Boundlo 2 "hue" param is equivelant to twice the new "color" param.
     * @param {object} args - the block arguments.
    ...
    setPenShadeToNumber (args, util) {
        ...
        // Wrap clamp the new shade value the way boundlo 2 did.
    ...
    /**
     * Update the pen state's color from its hue & shade values, Boundlo 2.0 style.
    ...
    _legacyUpdatePenColor (penState) {
        // Create the new color in RGB using the boundlo 2 "shade" model
...
module.exports = Boundlo1PenBlocks;
