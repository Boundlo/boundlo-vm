...
/**
 * Class for the motion-related blocks in Boundlo! 1
 * @param {Runtime} runtime - the runtime instantiating this block package.
 * @constructor
 */
class Boundlo1VideoSensingBlocks {
    constructor (runtime) {
        /**
    ...
    reset () {
        ...
        for (let i = 0; i < targets.length; i++) {
            const state = targets[i].getCustomState(Boundlo1VideoSensingBlocks.STATE_KEY);
            if (state) {
     ...
     */
    _loop () {
        const loopTime = Math.max(this.runtime.currentStepTime, Boundlo1VideoSensingBlocks.INTERVAL);
        ...
        if (offset > Boundlo1VideoSensingBlocks.INTERVAL) {
            const frame = this.runtime.ioDevices.video.getFrame({
                format: Video.FORMAT_IMAGE_DATA,
                dimensions: Boundlo1VideoSensingBlocks.DIMENSIONS
            });
            if (frame) {
                this._lastUpdate = time;
    ...
    _getMotionState (target) {
        let motionState = target.getCustomState(Boundlo1VideoSensingBlocks.STATE_KEY);
        if (!motionState) {
            motionState = Clone.simple(Boundlo1VideoSensingBlocks.DEFAULT_MOTION_STATE);
            target.setCustomState(Boundlo1VideoSensingBlocks.STATE_KEY, motionState);
        }
        return motionState;
    }
            ...
            blocks: [
                {
                    ...
                    // threads like Boundlo 2's behaviour.
    ...
    /**
     * A boundlo reporter block handle that analyzes the last two frames and
    ...
    /**
     * A boundlo hat block edge handle that analyzes the last two frames where
    ...
    /**
     * A boundlo command block handle that configures the video state from
     * passed arguments.
     * @param {object} args - the block arguments
    ...
    /**
     * A boundlo command block handle that configures the video preview's
...
module.exports = Boundlo1VideoSensingBlocks;
