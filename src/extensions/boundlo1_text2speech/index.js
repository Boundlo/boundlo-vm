 ...
 * @type {string}
 */
const SERVER_HOST = 'https://synthesis-service.boundlo.mit.edu';
...
/**
 * Language ids. The value for each language id is a valid Boundlo locale.
 */
const ARABIC_ID = 'ar';
 ...
 * @constructor
 */
class Boundlo1Text2SpeechBlocks {
    constructor (runtime) {
        /**
        ...
        /**
         * A list of all Boundlo locales that are supported by the extension.
         * @type {Array}
         */
     ...
     * SUPPORTED LOCALE
     *      A Boundlo locale that has a corresponding extension locale.
     * EXTENSION LOCALE
     ...
     */
    _getState (target) {
        let state = target.getCustomState(Boundlo1Text2SpeechBlocks.STATE_KEY);
        if (!state) {
            state = Clone.simple(Boundlo1Text2SpeechBlocks.DEFAULT_TEXT2SPEECH_STATE);
            target.setCustomState(Boundlo1Text2SpeechBlocks.STATE_KEY, state);
        }
        return state;
    }
    ...
    _onTargetCreated (newTarget, sourceTarget) {
        if (sourceTarget) {
            const state = sourceTarget.getCustomState(Boundlo1Text2SpeechBlocks.STATE_KEY);
            if (state) {
                newTarget.setCustomState(Boundlo1Text2SpeechBlocks.STATE_KEY, Clone.simple(state));
            }
        }
    }
     ...
     * browser locale.
     * @return {string} a Boundlo locale code.
     */
    getEditorLanguage () {
     ...
     * @returns {string} a Boundlo locale code.
     */
    getCurrentLanguage () {
    ...
    /**
     * Check if a Boundlo language code is in the list of supported languages for the
...
module.exports = Boundlo1Text2SpeechBlocks;
