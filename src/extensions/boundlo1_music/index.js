...
/**
 * Class for the music-related blocks in Boundlo! 1
 * @param {Runtime} runtime - the runtime instantiating this block package.
 * @constructor
 */
class Boundlo1MusicBlocks {
    constructor (runtime) {
        /**
    ...
    /**
     * An array that is a mapping from MIDI instrument numbers to Boundlo instrument numbers.
     * @type {number[]}
     */
    ...
    /**
     * An array that is a mapping from MIDI drum numbers in range (35..81) to Boundlo drum numbers.
    ...
    _getMusicState (target) {
        let musicState = target.getCustomState(Boundlo1MusicBlocks.STATE_KEY);
        if (!musicState) {
            musicState = Clone.simple(Boundlo1MusicBlocks.DEFAULT_MUSIC_STATE);
            target.setCustomState(Boundlo1MusicBlocks.STATE_KEY, musicState);
        }
        return musicState;
    }
    ...
    _onTargetCreated (newTarget, sourceTarget) {
        if (sourceTarget) {
            const musicState = sourceTarget.getCustomState(Boundlo1MusicBlocks.STATE_KEY);
            if (musicState) {
                newTarget.setCustomState(Boundlo1MusicBlocks.STATE_KEY, Clone.simple(musicState));
            }
        }
    }
     ...
     * This block is implemented for compatibility with old Boundlo projects that use the
    ...
    midiPlayDrumForBeats (args, util) {
        ...
        if (midiDescription) {
            ...
            drumNum = 2; // Default instrument used in Boundlo 2.0
        }
    ...
    _playDrumNum (util, drumNum) {
        ...
        if (this._concurrencyCounter > Boundlo1MusicBlocks.CONCURRENCY_LIMIT) {
            return;
        }
    ...
    playNoteForBeats (args, util) {
        if (this._stackTimerNeedsInit(util)) {
            ...
            note = MathUtil.clamp(note,
                Boundlo1MusicBlocks.MIDI_NOTE_RANGE.min, Boundlo1MusicBlocks.MIDI_NOTE_RANGE.max);
            let beats = Cast.toNumber(args.BEATS);
            beats = this._clampBeats(beats);
            // If the duration is 0, do not play the note. In Boundlo 2.0, "play drum for 0 beats" plays the drum,
     ...
     */
    _clampBeats (beats) {
        return MathUtil.clamp(beats, Boundlo1MusicBlocks.BEAT_RANGE.min, Boundlo1MusicBlocks.BEAT_RANGE.max);
    }
    ...
    /**
     * Select an instrument for playing notes according to a mapping of MIDI codes to Boundlo instrument numbers.
     * This block is implemented for compatibility with old Boundlo projects that use the 'midiInstrument:' block.
     * @param {object} args - the block arguments.
     * @param {object} util - utility object provided by the runtime.
     ...
     * the MIDI to Boundlo instrument mapping.
     ...
     */
    _updateTempo (tempo) {
        tempo = MathUtil.clamp(tempo, Boundlo1MusicBlocks.TEMPO_RANGE.min, Boundlo1MusicBlocks.TEMPO_RANGE.max);
...
module.exports = Boundlo1MusicBlocks;
