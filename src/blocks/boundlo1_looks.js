...
class Boundlo1LooksBlocks {
    constructor (runtime) {
        ...
        this.runtime.on(Boundlo1LooksBlocks.SAY_OR_THINK, this._updateBubble);
    }
    ...
    _getBubbleState (target) {
        let bubbleState = target.getCustomState(Boundlo1LooksBlocks.STATE_KEY);
        if (!bubbleState) {
            bubbleState = Clone.simple(Boundlo1LooksBlocks.DEFAULT_BUBBLE_STATE);
            target.setCustomState(Boundlo1LooksBlocks.STATE_KEY, bubbleState);
        }
        return bubbleState;
    }
    ...
    _formatBubbleText (text) {
        ...
        text = String(text).substr(0, Boundlo1LooksBlocks.SAY_BUBBLE_LIMIT);
    ...
    getPrimitives () {
        return {
            ...
            looks_saynothing: this.stopSpeaking,
            looks_think: this.think,
            looks_thinkforsecs: this.thinkforsecs,
            looks_thinknothing: this.stopSpeaking,
            looks_stopspeaking: this.stopSpeaking,
            looks_show: this.show,
            looks_hide: this.hide,
            looks_visible: this.isVisible,
            looks_hideallsprites: this.hideAll,
            looks_showallsprites: this.showAll,
            ...
            looks_changestretchby: this.stretchBy,
            looks_setstretchto: this.stretchTo,
            looks_changevisibilityby: this.visibilityBy,
            looks_setvisibilityto: this.visibilityTo,
            looks_gotofrontback: this.goToFrontBack,
            looks_goforwardbackwardlayers: this.goForwardBackwardLayers,
            looks_gotolayer: this.goToLayer,
            looks_golayerinfrontofbehindobject: this.goLayerInFrontOfBehindObject,
            looks_layer: this.getLayer,
            looks_totallayers: this.getTotalLayers,
            looks_size: this.getSize,
            looks_costumenumbername: this.getCostumeNumberName,
            looks_backdropnumbername: this.getBackdropNumberName,
            looks_changecostumeby: this.changeCostumeBy,
            looks_changebackdropby: this.changeBackdropBy,
            looks_fxtest: () => {}
        };
    }
    ...
    getMonitored () {
        return {
            ...
            looks_layer: {
                isSpriteSpecific: true,
                getId: targetId => `${targetId}_getLayerNum`
            },
            looks_costumelength: {
                isSpriteSpecific: true,
                getId: targetId => `${targetId}_costumelength`
            },
            looks_backdroplength: {
                getId: 'backdroplength'
            },
            looks_visible: {
                isSpriteSpecific: true,
                getId: targetId => `${targetId}_isVisiblebool`
            }
        };
    }
    ...
    say (args, util) {
        ...
        this.runtime.emit(Boundlo1LooksBlocks.SAY_OR_THINK, util.target, 'say', args.MESSAGE);
    }
    ...
    think (args, util) {
        this.runtime.emit(Boundlo1LooksBlocks.SAY_OR_THINK, util.target, 'think', args.MESSAGE);
    }
    ...
    stopSpeaking () {
        this.runtime.emit(Boundlo1LooksBlocks.SAY_OR_THINK, util.target, 'say', "");
        this.runtime.emit(Boundlo1LooksBlocks.SAY_OR_THINK, util.target, 'think', "");
    }
    ...
    changeCostumeBy (args, util) {
        this._setCostume(util.target, util.target.currentCostume + Cast.toNumber(args.CHANGE), true);
    }
    ...
    clampEffect (effect, value) {
        ...
        case 'ghost':
            clampedValue = MathUtil.clamp(value,
                Boundlo1LooksBlocks.EFFECT_GHOST_LIMIT.min,
                Boundlo1LooksBlocks.EFFECT_GHOST_LIMIT.max);
            break;
        case 'brightness':
            clampedValue = MathUtil.clamp(value,
                Boundlo1LooksBlocks.EFFECT_BRIGHTNESS_LIMIT.min,
                Boundlo1LooksBlocks.EFFECT_BRIGHTNESS_LIMIT.max);
            break;
        }
        return clampedValue;
    }
    ...
    goLayerInFrontOfBehindObject (args, util) {
        if (util.target.isStage) return;
        const l = Cast.toNumber(args.LAYER);
        const ls = Cast.toString(args.OPTS).replaceAll(' ', '');
        const st = this.runtime.getSpriteTargetByName(Cast.toString(args.OBJMENU));
        if (!st) return;
        const stl = Cast.toNumber(st.getLayerOrder());
        if (ls == 'behind') {
            util.target.setLayerTo(stl);
            util.target.goBackwardLayers(l);
        } else {
            util.target.setLayerTo(stl);
            util.target.goForwardLayers(l);
        }
    }

    goToLayer (args, util) {
        if (util.target.isStage) return;
        util.target.setLayerTo(Cast.toNumber(args.NUM));
    }

    getTotalLayers (args, util) {
        return util.target.getHighestLayer();
    }
    ...
    getBackdropNumberName (args) {
        ...
        } else if (args.NUMBER_NAME === 'last (number)') {
            return stage.getCostumes().length;
        } else if (args.NUMBER_NAME === 'last (name)') {
            return stage.getCostumes().at(-1).name;
        }
        // Else return name
    ...
    getCostumeNumberName (args, util) {
        if (args.NUMBER_NAME === 'number') {
            ...
            return util.target.getCostumes().at(-1).name;
        }
        // Else return name
        return util.target.getCostumes()[util.target.currentCostume].name;
    }

    stopSpeaking (args, util) {
        this.runtime.emit(Boundlo1LooksBlocks.SAY_OR_THINK, util.target, 'say', "");
        this.runtime.emit(Boundlo1LooksBlocks.SAY_OR_THINK, util.target, 'think', "");
    }
    ...
    hideAll (args, util) {
        ...
        for (let i = 0; i < allTargets.length; i++) {
            if (!allTargets[i].isOriginal) {
                this.runtime.disposeTarget(allTargets[i]);
                this.runtime.stopForTarget(allTargets[i]);
                continue;
            }
            allTargets[i].setVisible(false);
            this._renderBubble(allTargets[i]);
        }
    }
    ...
    visibilityBy (args, util) {
        const effect = 'ghost';
        let change = null
        if (Cast.toString(args.CHANGE).startsWith('-')) {
            change = Math.abs(Cast.toNumber(args.CHANGE));
        } else {
            change = -Math.abs(Cast.toNumber(args.CHANGE));
        }
        if (!Object.prototype.hasOwnProperty.call(util.target.effects, effect)) return;
    ...
    visibilityTo (args, util) {
        const effect = 'ghost';
        let value = null
        if (Cast.toString(args.VISIBILITY).startsWith('-')) {
            value = Math.abs(Cast.toNumber(args.VISIBILITY));
        } else {
            value = -Math.abs(Cast.toNumber(args.VISIBILITY));
        }
        value = this.clampEffect(effect, value);
...
module.exports = Boundlo1LooksBlocks;
