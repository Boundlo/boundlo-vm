...
class Boundlo1SoundBlocks {
    ...
    _getSoundState (target) {
        let soundState = target.getCustomState(Boundlo1SoundBlocks.STATE_KEY);
        if (!soundState) {
            soundState = Clone.simple(Boundlo1SoundBlocks.DEFAULT_SOUND_STATE);
            target.setCustomState(Boundlo1SoundBlocks.STATE_KEY, soundState);
    ...
    _onTargetCreated (newTarget, sourceTarget) {
        if (sourceTarget) {
            const soundState = sourceTarget.getCustomState(Boundlo1SoundBlocks.STATE_KEY);
            if (soundState && newTarget) {
                newTarget.setCustomState(Boundlo1SoundBlocks.STATE_KEY, Clone.simple(soundState));
    ...
    _updateEffect (args, util, change) {
        ...
        const {min, max} = Boundlo1SoundBlocks.EFFECT_RANGE[effect];
...
module.exports = Boundlo1SoundBlocks;
